...
func ScoreVideoClip(ctx context.Context, videoPath, sourceImagePath string, shot *storyboardShot) (*CoherenceScore, error) {
	if videoPath == "" {
...
func SelectBestScoredClip(ctx context.Context, outputDir string, clips []ShotClip, shot *storyboardShot) (*ShotClip, *CoherenceScore, error) {
	if len(clips) == 0 {
...
func scoreSingleClip(ctx context.Context, outputDir string, c *ShotClip, shot *storyboardShot) (*ShotClip, *CoherenceScore, error) {
	local, ok := publicURLToLocal(outputDir, c.FileURL)
	if !ok {
		return c, nil, fmt.Errorf("clip file not found")
	}
	sourceLocal := ""
	if c.SourceImageURL != "" {
		if p, ok := publicURLToLocal(outputDir, c.SourceImageURL); ok {
			sourceLocal = p
		}
	}
...
func scoreMetadataDimensions(shot *storyboardShot) (semantic, transition, style float64) {
