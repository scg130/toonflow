...
5. dialogue 只写本镜实际口播，格式「角色名：台词」；无对白留空；纯环境音写「环境音：描述」，不要混入角色台词；台词内容不要重复粘贴进 description；
