...
func ResolveShotImageCDNURL(ctx context.Context, db *sql.DB, v adapter.Vendor, outputDir, projectID, episodeID string, shot *storyboard.ShotMeta) (string, error) {
	if shot == nil {
	...
	local, ok := fsutil.PublicURLToLocal(outputDir, shot.ImageURL)
	if !ok {
	...
	if db != nil && projectID != "" && episodeID != "" {
		_ = storyboard.UpdateStoryboardShotMedia(db, projectID, episodeID, shot.ShotNumber, shot.ImageURL, remote)
	}
	return remote, nil
}
