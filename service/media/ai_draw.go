...
func GenerateImages(ctx context.Context, items []task.StoryboardItem, style, resolution, outputDir string, v adapter.Vendor) ([]task.ImageArtifact, error) {
	...
	for i, item := range items {
		...
		prompt := project.BuildShotImagePrompt(item, style, project.ResolutionToVideoRatio(resolution), "", "")
		...
		if strings.HasPrefix(resp.DataURL, "http://") || strings.HasPrefix(resp.DataURL, "https://") {
			if err := adapter.DownloadHTTPURL(ctx, localPath, resp.DataURL); err != nil {
