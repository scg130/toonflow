...
func ExtractAssetsFromEpisode(ctx context.Context, db *sql.DB, v adapter.Vendor, projectID, episodeID string) (int, error) {
	var script string
	...
	resp, err := v.TextRequest(ctx, adapter.DefaultTextModel, adapter.TextParams{
		Messages: []adapter.TextMessage{
			{Role: "system", Content: "从剧本中提取资产，输出 JSON 数组，每项含 name, type(role/scene/prop), desc"},
	...
	for _, it := range items {
		...
		_, err := db.Exec(`
			INSERT OR IGNORE INTO o_assets (project_id, name, desc, type)
			VALUES (?, ?, ?, ?)`, projectID, it.Name, it.Desc, it.Type)
		if err == nil {
			count++
		}
	}
