...
func (i *ChatActionIntent) Validate(episodeID string) error {
	...
	needsEpisode := map[string]bool{
		...
		"generate_storyboard": true, "extract_assets": true, "assign_character_voices": true,
		"generate_shot_image": true, "compose_shot": true, "batch_compose_shots": true,
	}
	...
	if i.Type == "compose_shot" {
		n, err := i.ShotNumber()
		if err != nil {
			return fmt.Errorf("合成对白需指定镜号：在 ACTION 前一行输出 SHOT:镜号")
		}
		if n <= 0 {
			return fmt.Errorf("镜号无效")
		}
	}
	return nil
}
...
func EnrichIntentFromUserMessage(intent *ChatActionIntent, userMsg string) {
	...
	if intent.Type != "generate_shot_image" && intent.Type != "compose_shot" {
		return
	}
...
- assign_character_voices — 为角色自动分配 TTS 音色
- generate_shot_image — 为指定镜号生成图片（SHOT:镜号 或 ACTION:generate_shot_image:镜号）
- compose_shot — 为指定镜号合成对白 TTS + 字幕（SHOT:镜号 或 ACTION:compose_shot:镜号）
- batch_compose_shots — 批量合成所有含对白镜头
